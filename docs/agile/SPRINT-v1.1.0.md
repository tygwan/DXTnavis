...
**Tasks:**
- [x] `TextBox_PreviewKeyDown`에 `Key.ImeProcessed` 처리 추가
- [x] OEM 키 확장 (특수문자 전체 지원)
- [x] 빌드 검증 통과

---

## 📊 기능 분석: AWP 4D 탭 vs Schedule 탭

### 워크플로우 비교

| 항목 | AWP 4D 탭 | Schedule 탭 |
|------|----------|-------------|
| **데이터 소스** | 외부 CSV 파일 (Browse) | 선택된 내부 객체 |
| **워크플로우** | 7단계 (CSV 필수) | 3단계 (직접 실행) |
| **Property Write** | ✅ ComAPI 지원 | ❌ 미지원 |
| **한글화** | ❌ 영문 전용 | ✅ TaskType 한글화 |
| **DateMode** | ❌ Planned만 | ✅ 3가지 옵션 |
| **ParentSet 전략** | 4개 (enum 기반) | 7개 (확장됨) |
| **Direct 실행** | ❌ CSV 필수 | ✅ v1.2.0 추가 |
...
│ AWP 4D 탭 (외부 CSV 기반)                                        │
├─────────────────────────────────────────────────────────────────┤
│ CSV File → Parse → Match → Property Write → Set → TimeLiner    │
│    ↓         ↓       ↓           ↓           ↓        ↓        │
│ [Browse]  [검증]  [SyncID]   [ComAPI]    [계층]   [Task]       │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│ Schedule 탭 (내부 객체 기반)                                     │
├─────────────────────────────────────────────────────────────────┤
│ Selected Objects → Preview → Direct Execute → Set → TimeLiner  │
│       ↓               ↓           ↓            ↓        ↓      │
│   [체크박스]      [DataGrid]  [ObjectId]    [계층]   [Task]    │
└─────────────────────────────────────────────────────────────────┘
```
...
| 시나리오 | 권장 탭 | 이유 |
|----------|---------|------|
| 외부 시스템 CSV 연동 (Revit 등) | **AWP 4D** | CSV 파싱 및 검증 기능 |
| Property Write 필요 | **AWP 4D** | ComAPI 지원 |
| 빠른 내부 작업 | **Schedule** | 직접 실행으로 3단계 완료 |
| 한글 UI 필요 | **Schedule** | TaskType 한글화 |
| 날짜 옵션 다양성 | **Schedule** | DateMode 3가지 |
| 상세 ParentSet 전략 | **Schedule** | 7가지 전략 지원 |

### 기능 중복 및 정리 방안

**현재 상태:**
- 두 탭 모두 TimeLiner Task 생성 가능
- Schedule 탭이 AWP 4D 탭의 핵심 기능 대부분 포함
- AWP 4D 탭만의 고유 기능: **Property Write (ComAPI)**

**향후 정리 방안 (Optional):**
1. **현행 유지**: 두 탭 모두 유지 (사용자 선택권)
2. **AWP 4D 탭 경량화**: Property Write + 외부 CSV 전용으로 축소
3. **기능 통합**: Schedule 탭에 CSV Import 기능 추가 후 AWP 4D 제거

**결론:** 현재는 **현행 유지** 권장
- AWP 4D: 외부 CSV, Property Write 전용
- Schedule: 내부 워크플로우, 직접 실행 전용
```
