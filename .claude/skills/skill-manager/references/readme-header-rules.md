...
README 헤더는 dev-doc-system의 문서 구조와 연동됩니다:

```
docs/
├── direction/ROADMAP.md    → 📊 개발 진행 상황 소스
├── status/CURRENT.md       → 현재 작업 상태 소스
├── planning/BACKLOG.md     → 다음 예정 작업 소스
├── feedback/ISSUES.md      → 활성 이슈 카운트
└── README.md               ← 헤더 업데이트 대상
```
...
→ 상세: [CURRENT.md](status/CURRENT.md) | [ISSUES.md](feedback/ISSUES.md)
...
| 문서 | 설명 |
|------|------|
| [VISION.md](direction/VISION.md) | 프로젝트 비전 및 목표 |
| [ROADMAP.md](direction/ROADMAP.md) | 개발 로드맵 |
| [DECISIONS.md](direction/DECISIONS.md) | 주요 기술 결정 (ADR) |
...
README 헤더 업데이트 시 자동으로 수집되는 정보:

| 데이터 | 소스 | 수집 방법 |
|--------|------|----------|
| Phase 진행률 | ROADMAP.md | Phase별 진행률 파싱 |
| 현재 작업 수 | CURRENT.md | 진행중 항목 카운트 |
| 완료 작업 수 | CURRENT.md | 완료 체크박스 카운트 |
| 활성 이슈 수 | ISSUES.md | 🔴 상태 카운트 |
| 백로그 P0 수 | BACKLOG.md | P0 항목 카운트 |

### 명령어 확장

```bash
# 전체 헤더 갱신 (dev-doc-system 연동)
/skill-manager update-header --sync-docs

# 특정 섹션만 업데이트
/skill-manager update-header --section progress
/skill-manager update-header --section status
/skill-manager update-header --section techstack

# 문서 링크 검증
/skill-manager update-header --validate-links
```
...
- [dev-doc-system Skill](../../dev-doc-system/SKILL.md)
